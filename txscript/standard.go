...
func ExtractP2XScriptSigType(sdb ScriptDB, chainParams *chaincfg.Params, pkScript []byte) ([]uint8, int, error) {
	class, addresses, required, err := ExtractPkScriptAddrs(DefaultScriptVersion, pkScript, chainParams)
	if err != nil {
		return []uint8{0}, 0, err
	}
	...
	case ScriptHashTy:
		...
		//Get the original script for the address,which is multisig type script
		script, err := sdb.GetScript(addresses[0])
		if err != nil {
			return []uint8{uint8(chainec.ECTypeSecp256k1)}, 0, err
		}
		return ExtractP2XScriptSigType(sdb, chainParams, script)
	case MultiSigTy:
		if sdb == nil {
			return []uint8{uint8(chainec.ECTypeSecp256k1)}, required, nil
		}
		...
		for _, addr := range addresses {
			switch addr.(type) {
			case *hcutil.AddressSecpPubKey:
				sigTypes = append(sigTypes, uint8(chainec.ECTypeSecp256k1))
			case *hcutil.AddressBlissPubKey:
				sigTypes = append(sigTypes, bs.BSTypeBliss)
			default:
				sigTypes = append(sigTypes, uint8(chainec.ECTypeSecp256k1))
			}
		}
...
func ExtractPkScriptAltSigType(pkScript []byte) (uint8, error) {
	...
	if !(isPKA || isPKHA) {
		return 0, fmt.Errorf("wrong script type")
	}
