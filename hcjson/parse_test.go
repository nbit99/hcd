...
func TestEncodeConcatenatedHashes(t *testing.T) {
	...
	for j := 0; j < len(hashSlice)+1; j++ {
		...
		// Encode to string
		concatenated := hcjson.EncodeConcatenatedHashes(hashSlice[:j])
		// Verify output
...
func TestDecodeConcatenatedHashes(t *testing.T) {
	...
	decodedHashes, err := hcjson.DecodeConcatenatedHashes(concatenatedHashes)
	if err != nil {
...
func TestEncodeConcatenatedVoteBits(t *testing.T) {
	...
	encodedResults, err := hcjson.EncodeConcatenatedVoteBits(testVbs)
	if err != nil {
...
func TestDecodeConcatenatedVoteBits(t *testing.T) {
	...
	decodedSlice, err :=
		hcjson.DecodeConcatenatedVoteBits(encodedBytesStr)
	if err != nil {
	...
	decodedSlice, err = hcjson.DecodeConcatenatedVoteBits(encodedBytesStr)
	if err == nil {
	...
	for _, str := range testStrings {
		...
		if !ok {
			t.Fatalf("DecodeConcatenatedHashes error is not "+
				"expected type *hcjson.RPCError: %T", err)
		}
